...
public:
    ...
    NeuralNetworkGraph() {
        ...
        k_pkt_to_stream = kernel::create(pkt_to_stream);  // Use simpler version without routing
        source(k_pkt_to_stream) = "pkt_to_stream.cpp";
        headers(k_pkt_to_stream) = {"pkt_to_stream.h"};
        ...
        // layer0_in -> k_packet -> splitter -> k_pkt_to_stream -> dense1 -> layer0_out
        connect<stream>(layer0_in.out[0], k_packet.in[0]);           // float stream → packet kernel
        connect<pktstream>(k_packet.out[0], splitter.in[0]);         // packet stream → splitter
        connect<pktstream>(splitter.out[0], k_pkt_to_stream.in[0]);  // splitter → pkt_to_stream
        connect<stream>(k_pkt_to_stream.out[0], dense1.inB[0]);      // float stream → dense
        connect<stream>(dense1.out[0], layer0_out.in[0]);            // dense output → output
