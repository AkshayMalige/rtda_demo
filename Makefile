...
# Golden reference and host output files for each graph
ifeq ($(GRAPH),aieml)
  GOLDEN_OUTPUT    := embed_model_output.txt
  HOST_OUTPUT_FILE := host_output.txt
else ifeq ($(GRAPH),aieml2)
  GOLDEN_OUTPUT    := solver_0_model_output.txt
  HOST_OUTPUT_FILE := solver_0_host_output.txt
else
  GOLDEN_OUTPUT    := output_model_output.txt
  HOST_OUTPUT_FILE := output_host_output.txt
endif
###########################################################################
...
$(XCLBIN): link host $(POST_BOOT) | $(PKG_DIR)
	...
	@if [ "$(TARGET)" = "hw_emu" ]; then \
	  ...
	  python3 verify_outputs.py $(PKG_DIR)/data/$(GOLDEN_OUTPUT) $(PKG_DIR)/data/$(HOST_OUTPUT_FILE); \
	fi
