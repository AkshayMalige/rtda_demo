#pragma once
#include <adf.h>
#include "include.h"   // defines INPUT_SIZE, HIDDEN_SIZE, VEC_WIDTH_D1, VEC_WIDTH_D2, OUTPUT_SIZE, FUSE_WIDTH_D2
 ...
 *      VEC_WIDTH_D1    = 4   lanes    (SIMD width of dense1)
 *      VEC_WIDTH_D2    = 4   lanes    (SIMD width of dense2)
 *      FUSE_WIDTH_D2   = 4   blocks   (blocks fused inside dense2)
 */
...
public:
  ...
  NeuralNetworkGraph() {
    ...
    constexpr int D1_IN_WINDOW  = INPUT_SIZE * HIDDEN_SIZE / VEC_WIDTH_D1;           // 192  floats
    constexpr int D1_W_WINDOW   = (INPUT_SIZE / 2) * HIDDEN_SIZE;                   // 384  floats

    constexpr int VEC_WIDTH_D2    = 4;       // AIEML float SIMD width (4 lanes)
    constexpr int FUSE_WIDTH_D2   = 4;       // four 4-lane blocks → 16 neurons



    /*  Dense-2 consumes each activation vector  (HIDDEN_SIZE)  FUSE_WIDTH_D2*VEC_WIDTH_D2 times */
    constexpr int D2_IN_REPS    = HIDDEN_SIZE / (FUSE_WIDTH_D2 * VEC_WIDTH_D2);      // 8    repetitions
