...
struct alignas(64) aligned_array {
    ...
    const T& operator[](std::size_t idx) const { return elems[idx]; }
};

// Read text file as doubles, cast to target type T (matches aieml/utils.hpp pattern)
template<typename T>
static std::vector<T> read_values(const std::string& path, std::size_t expected_count)
{
...
static aligned_array<DATA_TYPE, N> read_values_array(const std::string& path)
{
    auto values = read_values<DATA_TYPE>(path, N);
    aligned_array<DATA_TYPE, N> result{};
...
{
    auto payload = read_values_array<N>(path);
...
int main(int argc, char** argv)
{
    ...
    constexpr unsigned HOST_DEBUG_GMIO_BURST = 64U;

#ifdef USE_INT16
    std::cout << "[host] Precision: INT16" << std::endl;
#else
    std::cout << "[host] Precision: FLOAT32" << std::endl;
#endif
    ...
    try {
        ...
        auto in_ptr  = in_bo.map<DATA_TYPE*>();
